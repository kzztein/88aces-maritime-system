...
// ============================================================
function buildAntiPiracy(
    string $fn, string $mi, string $sn,
    string $day, string $dayOrd, string $month, string $year,
    string $cert, string $d
): string {
...
.name-text{display:inline-block;font-size:22pt;font-weight:bold;
           border-bottom:2px solid #111;padding:0 8mm 2mm;letter-spacing:1px;}
.body-p{text-align:center;font-size:10.5pt;color:#333;line-height:1.8;margin-bottom:2mm;}
.training-title{text-align:center;font-size:32pt;font-weight:bold;
...
function buildPDOS(
    string $fn, string $mi, string $sn,
    string $rank, string $principal,
    string $fullDate, string $cert, string $d
): string {
...
.page{width:210mm;min-height:297mm;padding:10mm 18mm 12mm;position:relative;overflow:hidden;}

.logo-wrap{text-align:center;margin-bottom:0;}
.logo-wrap img{height:20mm;}

.pdos-bg-wrap{position:relative;text-align:center;margin:2mm 0 0;height:48mm;overflow:hidden;}
.pdos-bg-wrap img.pdos-bg{
    position:absolute;left:50%;top:50%;
    width:105mm;opacity:1;z-index:0;
    margin-left:-52.5mm;margin-top:-24mm;
}
.pdos-titles{position:relative;z-index:1;padding-top:2mm;}
.pdos-titles .dept{font-size:9pt;color:#555;font-style:italic;letter-spacing:.3px;}
.pdos-titles .seminar{font-size:14pt;font-weight:900;color:#003087;
    text-transform:uppercase;letter-spacing:.5px;margin:1mm 0;}
.pdos-titles .cert-attend{font-size:12pt;font-weight:bold;color:#c0392b;
    text-transform:uppercase;letter-spacing:1px;text-decoration:underline;}

.info-section{margin:3mm 0 2mm;}
.info-row{display:flex;align-items:baseline;margin-bottom:2.5mm;font-size:10pt;}
.info-label{font-weight:bold;width:60mm;flex-shrink:0;}
.info-colon{width:6mm;flex-shrink:0;text-align:center;font-weight:bold;}
.info-value{flex:1;font-weight:bold;font-size:10.5pt;
    border-bottom:1px solid #000;min-height:5mm;padding-bottom:1px;}

.certify{font-size:9pt;font-weight:bold;color:#000;line-height:1.9;margin:5mm 0;text-align:justify;}
.certify u{text-decoration:underline;}

.sigs-table{width:100%;border-collapse:collapse;margin-top:10mm;}
.sigs-table td{width:50%;vertical-align:bottom;text-align:center;padding:0 10mm;}
.sig-spacer{height:22mm;}
.sig-img{height:18mm;display:block;margin:0 auto;}
.sig-name-line{border-top:1.5px solid #000;padding-top:2mm;}
...
<div class="pdos-bg-wrap">
  <img class="pdos-bg" src="{$pdosBg}">
  <div class="pdos-titles">
    <div class="dept">DEPARTMENT OF LABOR AND EMPLOYMENT</div>
    <div class="seminar">PRE-DEPARTURE ORIENTATION SEMINAR</div>
    <div class="cert-attend">CERTIFICATE OF ATTENDANCE</div>
  </div>
</div>

<div class="info-section">
  <div class="info-row">
    <div class="info-label">Name of OFW</div>
    <div class="info-colon">:</div>
    <div class="info-value">{$nameVal}</div>
  </div>
  <div class="info-row">
    <div class="info-label">Skill / Occupation</div>
    <div class="info-colon">:</div>
    <div class="info-value">{$rankVal}</div>
  </div>
  <div class="info-row">
    <div class="info-label">Country of Destination</div>
    <div class="info-colon">:</div>
    <div class="info-value">WORLD WIDE</div>
  </div>
  <div class="info-row">
    <div class="info-label">Local Recruitment Agency</div>
    <div class="info-colon">:</div>
    <div class="info-value">88 ACES MARITIME SERVICES INC.</div>
  </div>
  <div class="info-row">
    <div class="info-label">Foreign Principal</div>
    <div class="info-colon">:</div>
    <div class="info-value">{$principalVal}</div>
  </div>
  <div class="info-row">
    <div class="info-label">Foreign Employer</div>
    <div class="info-colon">:</div>
    <div class="info-value">{$principalVal}</div>
  </div>
</div>
...
<div class="certify">
  ...
  for the above program, held on <u>{$dateVal}</u> with Certificate No.
  <u>PDOS-{$certNo}</u>.
</div>
...
// ============================================================
function buildSECAT(
    string $fullName, string $fullDate,
    string $facilitator, string $cert, string $d
): string {
...
.name-text{display:inline-block;font-size:22pt;font-weight:bold;color:#111;
           border-bottom:2px solid #111;padding:0 6mm 2mm;}
